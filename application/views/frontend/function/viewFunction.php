...
?>
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<h1 class="h3 mb-3">View Function</h1>
			...
			$(document).ready(function() {
				...
				var clearHTML1 ="";
				clearHTML1 = '<table id="example" class="table" style="width:100%"><thead><tr><th>Id</th><th>Function name</th><th>Cost Center</th><th>Status</th><th>is_active_value</th><th>Option</th></tr></thead><tbody>';
				//all data call
				
				$form = $('#view_function_form');
				view_function($form);
				function view_function($form) {
					var formdata = new FormData($form[0]);
					formdata.append('form_name', 'view_function_form');
					$.ajax({ 
						type: "GET",
						cache : false,
						async: true,
						...
						success: function(data, response){
							...
							$.each(data, function (i, item) {
								...
								clearHTML1 += '<td id="func_id" class="func_id">'+item.func_id+'</td>';
								clearHTML1 += '<td id="func_name" class="func_name">'+item.func_name+'</td>';
							...
							$("#example tbody").on("click",'tr', function() {
												  ...
												  '<div class="modal-dialog">'+
													'<div class="modal-content">'+
													  '<div class="modal-header">'+
														'<h5 class="modal-title" id="staticBackdropLabel">Edit Function Details</h5>'+
														'<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>'+
													  '</div>'+
													  '<div class="modal-body">'+
														'<div class="row">'+
															'<div class="col-12">'+
																'<input type="hidden" class="form-control" id="func_id_edit" name="func_id_edit" value="'+func_id+'">'+
																'<input type="hidden" class="form-control" id="is_active_value_edit" name="is_active_value_edit" value="'+is_active+'">'+
																'<div class="">'+
																	'<label class="form-label">Function Name</label>'+
																	'<input type="text" class="form-control" id="func_name_edit" name="func_name_edit" value="'+func_name+'">'+
													  ...
													  '<div class="modal-body">'+
														'<div class="row">'+
															'<div class="col-12">'+
																...
																'<div class="">'+
																	'<label class="form-label">Function Name</label>'+
																	'<input type="text" class="form-control" id="func_name_edit" name="func_name_edit" value="'+func_name+'" disabled>'+
																	'<span style="color:red;position:absolute;font-size: 12px" class="error_comp_name"></span>'+
																'</div>'+
																'<div class="">'+
																'<label class="form-label">Cost Center</label>'+
								...
								var func_id = $("#func_id_edit").val();
								var func_name = $("#func_name_edit").val();
								var cost_center = $("#cost_center_select").val();
																								
								
								if(typeof func_id !== 'undefined' && func_id !== '' && typeof func_name !== 'undefined' && func_name !== '' 
								&& typeof cost_center !== 'undefined' && cost_center !== ''){
									...
									var functionDetails = JSON.stringify({
										...
										$form = $('#update_function_form');
										update_function($form);
									
									function update_function($form) {
										var formdata = new FormData($form[0]);
										formdata.append('form_name', 'update_function_form');
										$.ajax({
											type: 'PUT',
											dataType: 'json',
											contentType: 'application/json',
											url: 'https://localhost:44371/api/functions/'+func_id,
