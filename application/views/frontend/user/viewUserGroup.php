...
?>
			<main class="content">
				<div class="container-fluid p-0">
					...
					<h1 class="h3 mb-3">View User Group</h1>
			...
			$(document).ready(function() {
				...
				var clearHTML1 ="";
				clearHTML1 = '<table id="example" class="table" style="width:100%"><thead><tr><th>Id</th><th>Group name</th><th>Group Description</th><th>Status</th><th>is_active_value</th><th>Option</th></tr></thead><tbody>';
				//all data call
				
				$form = $('#view_user_group_form');
				view_user_group($form);
				function view_user_group($form) {
					var formdata = new FormData($form[0]);
					formdata.append('form_name', 'view_user_group_form');
					$.ajax({ 
						type: "GET",
						cache : false,
						async: true,
						...
						success: function(data, response){
							...
							$.each(data, function (i, item) {
								...
								clearHTML1 += '<td id="grp_id" class="grp_id">'+item.grp_id+'</td>';
								clearHTML1 += '<td id="grp_name" class="grp_name">'+item.grp_name+'</td>';
								clearHTML1 += '<td id="grp_desc" class="grp_desc">'+item.grp_desc+'</td>';
								if(item.is_active == 1){
							...
							$("#example tbody").on("click",'tr', function() {
												  ...
												  '<div class="modal-dialog">'+
													'<div class="modal-content">'+
													  '<div class="modal-header">'+
														'<h5 class="modal-title" id="staticBackdropLabel">Edit User Group Details</h5>'+
														'<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>'+
													  '</div>'+
													  '<div class="modal-body">'+
														'<div class="row">'+
															'<div class="col-12">'+
																'<input type="hidden" class="form-control" id="grp_id_edit" name="grp_id_edit" value="'+grp_id+'">'+
																'<input type="hidden" class="form-control" id="is_active_value_edit" name="is_active_value_edit" value="'+is_active+'">'+
																'<div class="">'+
																	'<label class="form-label">Group Name</label>'+
																	'<input type="text" class="form-control" id="grp_name_edit" name="grp_name_edit" value="'+grp_name+'">'+
																...
																'<div class="">'+
																	...
																	'<input type="text" class="form-control" id="grp_desc_edit" name="grp_desc_edit" value="'+grp_desc+'">'+
																	'<span style="color:red;position:absolute;font-size: 12px" class="error_comp_name"></span>'+
																'</div>'+																
																'<div class="">'+
													  ...
													  '<div class="modal-body">'+
														'<div class="row">'+
															'<div class="col-12">'+
																...
																'<div class="">'+
																	'<label class="form-label">Group Name</label>'+
																	'<input type="text" class="form-control" id="grp_name_edit" name="grp_name_edit" value="'+grp_name+'" disabled>'+
																	'<span style="color:red;position:absolute;font-size: 12px" class="error_comp_name"></span>'+
																'</div>'+
																'<div class="">'+
																	'<label class="form-label">Group Description</label>'+
																	'<input type="text" class="form-control" id="grp_desc_edit" name="grp_desc_edit" value="'+grp_desc+'" disabled>'+
																	'<span style="color:red;position:absolute;font-size: 12px" class="error_comp_name"></span>'+
																'</div>'+																
																'<div class="">'+
																	'<label class="form-check">'+
								...
								var grp_id = $("#grp_id_edit").val();
								var grp_name = $("#grp_name_edit").val();
								var grp_desc = $("#grp_desc_edit").val();																								
								
								if(typeof grp_id !== 'undefined' && grp_id !== '' && typeof grp_name !== 'undefined' && grp_name !== '' 
								&& typeof grp_desc !== 'undefined' && grp_desc !== ''){
									...
									function update_group($form) {
										var formdata = new FormData($form[0]);
										formdata.append('form_name', 'update_group_form');
										$.ajax({
											type: 'PUT',
											dataType: 'json',
											contentType: 'application/json',
											url: 'https://localhost:44371/api/usergroups/'+grp_id,
											data: groupDetails,
											cache: false,
											processData: false,
											crossDomain: true,
